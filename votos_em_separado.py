...
def getVotosEmSeparado(meetings, comission):
    ...
    for meeting in meetingsOfComission:
        ...
        def processItem(item):
            nome = item.get("nome")
            descricaoResultado = item.get("descricaoResultado", "")
            resultadoNormalizado = str(descricaoResultado).lower()
            hasVotoEmSeparado = "destaque" in resultadoNormalizado

            if hasVotoEmSeparado:
                pls.append(nome)
                print(descricaoResultado)

        def processParte(parte):
            if (
                meeting.get("realizada") == "true"
                and parte
                and parte.get("isDeliberativa") == "true"
            ):
...
def process_file(filepath):
    ...
    resultado = getVotosEmSeparado(data, "CCJ")
    return resultado
...
for filename in os.listdir("."):
    if filename.endswith("json"):
        ...
        if type(resultado) is list and len(resultado) > 0:
