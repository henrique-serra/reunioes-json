...
def getReqsInfoOfComission(meetings, comisisonId):
  ...
  for meeting in meetingsOfComission:
    ...
    def processItem(item):
      nome = item.get('nome')
      isReqInfo = 'Aprovada' in item.get('resultado').get('descricao')

      if isReqInfo:
        reqs_informacao.append(nome)
  ...
  return reqs_informacao

def process_file(filepath):
  with open(filepath, 'r', encoding='utf-8') as f:
    data = json.load(f)

  resultado = getReqsInfoOfComission(data, '59')
  return resultado
