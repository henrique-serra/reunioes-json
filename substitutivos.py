...
def getSubstitutivoOfComission(meetings, comisisonId):
  ...
  for meeting in meetingsOfComission:
    ...
    def processItem(item):
      nome = item.get('nome')
      isTerminativo = item.get('isTerminativo') == 'true'
      isSubstitutivo = 'substitutivo' in item.get('resultado').get('descricao')

      if isTerminativo and isSubstitutivo:
        substitutivos.append(nome)
...
def process_file(filepath):
  ...
  resultado = getSubstitutivoOfComission(data, '59')
  return resultado
